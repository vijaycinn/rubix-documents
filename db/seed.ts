...
export function seedDatabase() {
  const stmt = db.prepare(`
    INSERT OR IGNORE INTO priority_items (
      item_number, recommendation, pillar, effort, cost_impact, business_value, priority_level, category,
      technical_category, description, implementation_steps, dependencies, technical_notes, related_services, compliance_notes
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);

  const priorityItems = [
    // CRITICAL PRIORITY (0-30 Days) - Item 1
    [
    ...
    // HIGH PRIORITY (30-60 Days) - Item 8
    [
    ...
    // MEDIUM PRIORITY (60-90 Days) - Item 15
    [
  ...
  const count = db.prepare('SELECT COUNT(*) as count FROM priority_items').get() as { count: number };
  ...
  if (count.count === 0) {
    ...
    console.log('Database seeded successfully with', priorityItems.length, 'items (enhanced with technical details)');
  } else {
