...
**Project:** Records Management System (RMS) on Azure Government Cloud

---

<Note type="info" title="Azure Cloud Terminology Clarification">
Microsoft offers two distinct Azure cloud platforms: **Azure Commercial** (also called Azure Global or Azure Public) and **Azure for US Government** (also called Azure Government). There is no Microsoft product called "Azure GCC" or "Azure Commercial GCC" — this is a common source of confusion because "GCC" does not indicate which Azure platform is being used. The designation "GCC" refers to a Microsoft 365 Government Community Cloud tenant, which can be paired with either Azure Commercial or Azure Government.

For CJIS-regulated workloads, the choice between Azure Commercial and Azure Government has direct compliance implications. Azure Government enforces US citizenship screening for Microsoft personnel who may access customer data, which can make certain technical controls (such as Customer-Managed Keys) a **recommendation** rather than a **requirement**. On Azure Commercial, the absence of that personnel screening may elevate some controls — including CMK — from recommended to **required** depending on State-level CJIS policy interpretation. CJN Dakota should confirm with the MN DPS/BCA which platform and personnel screening baseline applies to their deployment.

Reference: [Azure support for public safety and justice (Azure Government)](https://learn.microsoft.com/en-us/azure/azure-government/documentation-government-overview-jps)
</Note>
...
|-----------|-------------|
| **Secure Data Management** | CJIS-compliant encryption at rest/transit, immutable audit logs, data residency in Azure Government |
...
| **Data** | TDE + CMK (SQL) | 5.10.1 | Recommended† |
| **Data** | CMK (Blob Storage) | 5.10.1 | Not Assessed |
...
<Note type="warning" title="† TDE + CMK: Recommended vs. Required Depends on Cloud Platform">
**Azure Government:** CMK is Recommended — Azure Government's built-in US citizenship screening for Microsoft personnel provides a baseline personnel security control that satisfies many CJIS requirements without mandating CMK.

**Azure Commercial:** CMK may be **Required** rather than Recommended. Azure Commercial does not enforce the same US citizenship screening, meaning technical controls like CMK (and Confidential Compute enclaves for encryption in-use) may become necessary compensating controls to meet CJIS 5.10.1. **CJN Dakota must determine whether MN State-level CJIS requirements change CMK from recommended to required for their chosen cloud platform.** Microsoft cannot make this determination on the customer's behalf.

**Blob Storage CMK scope:** The current architecture documents CMK for SQL Database (TDE) but does not explicitly confirm whether Azure Blob Storage (used for case documents and audit log archival) will also have CMK applied. This should be clarified — if Blob stores CJI data, CMK is subject to the same Commercial vs. Government CMK determination above.
</Note>
...
<Tabs defaultValue="recommendations">
  ...
  <TabsContent value="recommendations">
    ...
    | **TDE with CMK (SQL)** | Default encryption | Customer-managed keys via KV Premium | 2 weeks | 5.10.1 |
    | **CMK (Blob Storage)** | Platform-managed keys | Customer-managed keys for CJI blob containers | 1-2 weeks | 5.10.1 |
...
| **Azure Policy Compliance** | MISSING | Medium | Deploy NIST SP 800-53 and FedRAMP Moderate Azure Policy initiatives — no pre-built CJIS initiative exists; use [FBI CJIS-to-NIST 800-53 mapping](https://www.fbi.gov/file-repository/cjis/csp-v5_5-to-nist-controls-mapping-1.pdf) to validate coverage |
...
- [Azure support for public safety and justice (Azure Government)](https://learn.microsoft.com/en-us/azure/azure-government/documentation-government-overview-jps)
...
- [FBI CJIS Security Policy to NIST 800-53 Control Mapping](https://www.fbi.gov/file-repository/cjis/csp-v5_5-to-nist-controls-mapping-1.pdf)
